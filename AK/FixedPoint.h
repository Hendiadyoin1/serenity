...
template<size_t precission, Integral Underlying = i32>
class FixedPoint {
    using This = FixedPoint<precission, Underlying>;
    constexpr static Underlying radix_mask = (1 << precission) - 1;

public:
    constexpr FixedPoint() = default;
    template<Integral I>
    constexpr FixedPoint(I value)
        : m_value(value << precission)
    {
    }

    template<FloatingPoint F>
    constexpr FixedPoint(F value)
        : m_value((Underlying)(value * (1u << precission)))
    {
    }
    ...
    {
        return (F)m_value * pow<F>(0.5, precission);
    }
    template<Integral I>
    explicit constexpr operator I() const
    {
        I value = m_value >> precission;
        // fract(m_value) >= .5?
        if (m_value & (1u << (precission - 1))) {
            // fract(m_value) > .5?
    ...
    {
        return create_raw((m_value & ~radix_mask)
            + (m_value & radix_mask ? 1 << precission : 0));
    }
    constexpr This trunk() const
    {
        return create_raw((m_value & ~radix_mask)
            + ((m_value & radix_mask)
                    ? (m_value > 0 ? 0 : (1 << precission))
                    : 0));
    }
    ...
    constexpr This operator*(This const& other) const
    {
        ...
        This ret {};
        ret.raw() = value >> precission;
        // fract(value) >= .5?
        if (value & (1u << (precission - 1))) {
            // fract(value) > .5?
    ...
    constexpr This operator/(This const& other) const
    {
        ...
        return create_raw((m_value / other.m_value) << (precission));
    }

    template<Integral I>
    constexpr This operator+(I other) const
    {
        return create_raw(m_value + (other << precission));
    }
    template<Integral I>
    constexpr This operator-(I other) const
    {
    ...
    {
        return m_value >> precission == other && !(m_value & radix_mask);
    }
    template<Integral I>
    bool operator!=(I other) const
    {
    ...
    {
        if (m_value > 0)
            return (m_value >> precission) > other || (m_value >> precission == other && (m_value & radix_mask));
        if (other > 0)
            return false;
    ...
    // Casting form a float should be faster than casting to a float
    template<FloatingPoint F>
...
template<size_t precission, Integral Underlying>
struct Formatter<FixedPoint<precission, Underlying>> : StandardFormatter {
    Formatter() = default;
    ...
    void format(FormatBuilder& builder, FixedPoint<precission, Underlying> value)
    {
